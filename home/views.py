...
def product(request):
    ...
    mygoods_goods = mygoods.get_page(next_page_prod)
    context = {
        "mygoods_goods": mygoods_goods,
    }
...
def signin(request):
    if request.method == "POST":
        ...
        if user is not None:
            login(request, user)
            messages.success(request, f"Welcome {username} to FlickBusters, we hope you have the best experience!")
            return redirect("index")
        else:
...
def pay(request):
    if request.method == "POST":
        api_key = "sk_test_942f2fb8157788842e0edcca251d0236d2c09406"
        ...
        data = {"reference": ref, "order_number":shop_code, "callback_url": cburl, "email":email, "amount": int(total), "currency": "NGN"}
...
def callback(request):
    ...
    basket = Shopcart.objects.filter(user__username=request.user.username, paid=False) 
    receipt = Payment.objects.filter(user__username=request.user.username, paid=True) 
